...
public class Frm_HoaDonDatTour extends JPanel implements ActionListener, MouseListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj.equals(btnSuaHD)) {
			...
			if(selectedIndex == -1) {
				JOptionPane.showMessageDialog(btnSuaHD, "Hãy chọn dòng cần sửa !");
			}else {//chọn dòng cần sửa và nhấn nút
				listHD = dsHoaDon.getAllHoaDon();
				Frm_SuaHoaDonDatTour frmSua = new Frm_SuaHoaDonDatTour(this, isFocusCycleRoot());
				frmSua.setEditData(listHD.get(selectedIndex));
	...
	public void tongTien(){
		 Double total = 0.0;
		    for (int i = 0; i < tableHD.getRowCount(); i++){
		        Double amount = Double.parseDouble(modelTableHD.getValueAt(i, 9).toString().replace(",", ""));
		        total += amount;
		    }
	...
	private void themDuLieuHoaDonVaoBang() {
		...
		for (HoaDon obj : listHD) {
			if(obj.isTrangThai()==false) {
				tt = "Chưa thanh toán";
			}
			else {
				tt = "Đã thanh toán";
			}
			modelTableHD.addRow(new Object[] {obj.getMaHoaDon(), obj.getTour().getMaTour(), obj.getKhachHang().getMaKhachHang(),
					obj.getNhanVien().getMaNhanvien(), obj.getNgayDat(), obj.getSoLuongNguoiLon(), obj.getSoLuongTreEm(),
					obj.getPhuongThucThanhToan(), tt, df.format(obj.getTongTien())});
		}
	}
